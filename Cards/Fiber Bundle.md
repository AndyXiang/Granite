...
3. A topological space $F$ called the **typical fiber**.
4. A [[Map#Properties|surjection]] $\pi:E\to B$ satisfying $\pi ^{-1}(p)\cong F$, called the **projection**. The inverse image $\pi ^{-1}(p)=F_{p}$ is called the fiber at $p\in B$.
