...
where $\lambda$ is a [[Partition|partition]] of $n$ with $k$ cells. $D^{(\lambda)}$ is the irreducible [[Polynomial Representation|polynomial representation]] of $\mathrm{GL}(n,\mathbb{C})$ labeled by $\lambda$, and $S^{(\lambda)}$ is the [[Specht Module|Specht module]] of $S_{n}$ also labeled by $\lambda$. Moreover, dimension of $D^{(\lambda)}$ equals to the number of [[Young Tableau|semistandard Young tableaux]] and dimension of $S^{(\lambda)}$ equals to the number of [[Young Tableau|standard Young tableaux]]. More concisely, these numbers are given by [[Cards/Young Diagram|hook numbers]]:
