...
3. a *composition rule* for morphisms, such that whenever a codomain of a morphism $f$ matches the domain of a morphism $g$, there is a morphism $g\circ f$ called their composition. The composition is associative: $h\circ(g\circ f)=(h\circ g)\circ f$ and unital: $\mathrm{id}\circ f=f\circ\mathrm{id}=f$.
...
\begin{document}
	\begin{tikzcd}
		&&&\\
		w \ar[r,"f"] \ar[rr, bend left=50, "f\circ g"] \ar[loop left, "\mathrm{id}_w"]& x  \ar[r,"g"] \ar[rr, bend right=50, "g\circ h"] & y  \ar[r, "h"] & z  \ar[loop right, "\mathrm{id}_z"]\\
		&&&
	\end{tikzcd}
\end{document}
```
