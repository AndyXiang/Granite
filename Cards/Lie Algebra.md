...
Let $x_{i},i=1,2,\cdots$ be basis of $\mathfrak{g}$. **Structure constant** is defined by $[x_{i},x_{j}]=C_{ij}^{~~~~k}x_{k}$.
...
>[!tip] Theorem
>For any real or complex finite-dimensional Lie algebra $\mathfrak{g}$, there is a unique (up to [[Isomorphism|isomorphism]]) [[Connected Space|connected]] and [[Connected Space|simply connected]] [[Lie Group|Lie group]] $G$ whose [[Tangent Space|tangent space]] at the identity is $\mathfrak{g}$. Any other connected Lie group $G'$ with Lie algebra $\mathfrak{g}$ must be of the form $G /Z$ for some discrete [[Center|central]] [[Subgroup|subgroup]] $Z\lhd G$. 
