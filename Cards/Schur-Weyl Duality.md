...
Concretely, for $k,n\in \mathbb{N}$ consider the [[Tensor Product of Vector Space|tensor product]] of $n$ copies of $k$-dimensional [[Vector Space|vector space]] $\mathbb{C}^{k}$ as a $k\times n$-dimensional vector space: $\otimes^{n}\mathbb{C}^{k}$. As such, this carries:
1. a canonical linear representation of $\mathrm{GL}(k,\mathbb{C})$ as the $n$-fold [[Direct Product of Representation|product]] of its [[Fundamental Representation|fundamental representation]] on $\mathbb{C}^{k}$.
